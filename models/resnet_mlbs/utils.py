...
# bernoulli sampler with reparam trick
# UNIF_SAMPLER = tf.random_uniform_initializer(minval=0, maxval=1)
# UK = tf.constant(UNIF_SAMPLER((1, 200)))
...
def load_model_weights(fold_no, path):
...
def extract_mask_from_weights(weights, UK, filtering_slope=5, sampler_slope=10, sparse_ratio=0.1500):
    ...
    mask = np.array((N > UK)[0].numpy().astype(float))

    return mask


def save_evaluation_results(scores, preds, test, fold_no, selection, path):
    # with open(str(path)+'\\scores'+str(fold_no)+'.csv','w') as f:
    with open(str(path)+'scores_'+str(fold_no)+ '_' + str(selection) +'.csv','w') as f: #*GoogleColab
        w = csv.writer(f)
    ...
    np.save(str(path)+'preds_'+str(fold_no)+ '_' + str(selection) +'.npy', preds) #*GoogleColab
    np.save(str(path)+'test_'+str(fold_no)+ '_' + str(selection) +'.npy', test) #*GoogleColab
